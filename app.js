...
const PLANETS = [
  ...
  { name: "Jupiter", a: 5.20,  period: 11.86, color: 0xd8b48a, r: 6.0e9 },
  { name: "Saturn",  a: 9.58,  period: 29.46, color: 0xf2df9b, r: 5.0e9, ring: true },
...
// Simple starfield
function addStars(n = 1000, radius = 4e13) { // ~400 AU in meters
  ...
  const geom = new THREE.BufferGeometry();
  geom.setAttribute('position', new THREE.BufferAttribute(positions, 3));
  const mat = new THREE.PointsMaterial({ size: radius*0.002, sizeAttenuation:true, color: 0xffffff, depthWrite:false });
...
//      const sprite = new THREE.TextureLoader().load("textures/disc.png");
    ...
    pts.geometry.setAttribute("position", new THREE.BufferAttribute(vertices, 3));
...
//    with: { type: "json" },
//  });
  const { default: elements } = await import("/neo.json", { with: { type: "json" } });
